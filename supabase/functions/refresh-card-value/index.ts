...
Deno.serve(async (req) => {
  ...
  const cardRow = {
    ...
    parallel_type: (card as any).parallel_name,
    is_auto:      mcd.is_auto,
  ...
  console.log(`[refresh-card-value] query: "${query}"`);
  ...
  console.log(`[refresh-card-value] ${raw.length} raw → ${items.length} filtered`);
  ...
  return new Response(
    JSON.stringify({ value: avgValue, soldCount: prices.length, query }),
