...
void main() {
  ...
  vec3 bg;
  vec3 barFallback = mix(vec3(0.92), uGlassColor.rgb, uGlassColor.a);
  if (uHasBackground > 0.5) {
    vec2 sampleUv = localRefracted / uBackgroundSize;
    vec4 tex;
    if (uChromaticAberration < 0.001) {
      tex = texture(uTexture, sampleUv);
      bg = tex.rgb;
    } else {
      vec4 colR = texture(uTexture, (localRefracted + chromaticShift) / uBackgroundSize);
      vec4 colG = texture(uTexture, sampleUv);
      vec4 colB = texture(uTexture, (localRefracted - chromaticShift) / uBackgroundSize);
      tex = vec4(colR.r, colG.g, colB.b, colG.a);
      bg = tex.rgb;
    }
    float luma = dot(bg, vec3(0.299, 0.587, 0.114));
    if (tex.a < 0.12 || luma < 0.05) {
      bg = barFallback;
    }
  } else {
  ...
  if (uHasBackground > 0.5) {
    // Tab-bar lens: icons are painted *under* this layer. Keep the center
    // nearly clear so you see live icons; only edges get a light frost.
    // (High alpha here stacked on icons + the solid pill below = muddy/dark.)
    vec3 finalColor = bg + vec3(0.12 * edgeInfluence + 0.06 * fresnel);
    finalColor += rimColor * borderMask * 0.35;
    finalColor = min(finalColor, vec3(1.15));

    float glassAlpha = mix(0.04, 0.28, edgeInfluence);
    glassAlpha = mix(glassAlpha, 0.42, borderMask * 0.25);
    float alpha = glassAlpha * mask;
    fragColor = vec4(finalColor * alpha, alpha);
    return;
  }

  // Synthetic frost / non-capture path (segmented controls, etc.)
  ...
  float standardEdgeAlpha =
      uEdgeAlphaMultiplier * mix(0.3, 1.0, uInteractionIntensity);
  float glassAlpha = mix(standardBaseAlpha, standardEdgeAlpha, edgeInfluence);
