...
class _GlassEffectState extends State<GlassEffect>
  ...
  /// Capture source for the refraction shader ticker.
  GlobalKey? get _captureKey => widget.backgroundKey ?? _cachedScopeKey;

  void _updateTicker() {
    final bool shouldCapture =
        widget.interactionIntensity > 0.01 && _captureKey != null;
    if (shouldCapture) {
      if (!_ticker.isActive) {
        _ticker.start();
        // debugPrint(
  ...
  Widget build(BuildContext context) {
    ...
    // Path B: Native Impeller (Premium only) — screen-wide backdrop capture.
    // Tab-bar indicators pass an explicit [backgroundKey] on the icon layer;
    // those must use the refraction shader below, not the shared backdrop.
    final bool usesLocalRefractionSource = widget.backgroundKey != null;
    if (isImpeller &&
        widget.quality == GlassQuality.premium &&
        !usesLocalRefractionSource) {
    ...
    if (canUseRefraction && shader != null) {
      // Wait for the first icon-layer snapshot before painting glass — avoids
      // one frame of synthetic frost (reads as a white flash).
      if (usesLocalRefractionSource &&
          widget.interactionIntensity > 0.02 &&
          _backgroundImage == null) {
        return ClipPath(
      ...
      return ClipPath(
        clipper: ShapeBorderClipper(shape: widget.shape),
        clipBehavior: usesLocalRefractionSource
            ? Clip.antiAlias
...
class _RenderInteractiveIndicator extends RenderProxyBox {
  ...
  void paint(PaintingContext context, Offset offset) {
    if (child != null) {
      // Tab indicator already refracts a captured icon layer; extra backdrop
      // blur only darkens the morph.
      final blurSigma = (_backgroundKey != null && _backgroundImage != null)
          ? 0.0
          : _settings.effectiveBlur;
      if (blurSigma > 0) {
        context.pushLayer(
          BackdropFilterLayer(
